...
z = np.polyfit(V, I, 12) # 15차 다항식으로 V-I 그래프 근사
...
wl_R, tm_R = [], []                 # wavelength reference, transmission reference
...
for i in root.iter():
    if i.tag == 'Modulator':        # 태그 이름이 'Modulator'인 경우
        if i.attrib.get('Name') == 'DCM_LMZC_ALIGN':  # 속성 이름이 'DCM_LMZC_ALIGN'인 경우
            ...
            # 그래프를 그리기 위해 reference 데이터 플롯
            # plt.plot(wl_R, tm_R, color='#ED2B2A', linestyle=':', label='Reference')
...
plt.xlabel('Wavelength[nm]', labelpad=4, fontdict=font_axis)       # Write a label with a setting of axis
plt.ylabel('Measured transmission[dB]', labelpad=4, fontdict=font_axis)    # Write a label with a setting of axis
plt.xticks(fontsize=6)     # Set the font size of axis value
plt.yticks(fontsize=6)     # Set the font size of axis value
plt.legend(loc='lower center', ncol=2, fontsize=5)

plt.subplots_adjust(wspace=0.3, hspace=0.3)

# plt.text(0.2, 0.3, f"y:{fit_eq}", fontsize = 6,transform=plt.gca().transAxes)

# R-square 높은 reference - Raw data

rsq_ref = []   # rsq 값을 저장할 빈 리스트 생성
wl_list, tm_list = [], []
wl_ref, tm_ref = [], []
DC_bias = -2.0
...
for p in range(2, 9):   # 2~8차 다항식으로 반복
    # 다항식 회귀하여 참조 스펙트럼에 대한 회귀식 구하기
    fit = np.polyfit(np.array(wl_ref), np.array(tm_ref), p)
    fit_eq = np.poly1d(fit)   # 회귀식을 함수 형태로 저장
    print(f'[Fitting equation(ref)-{p}th]')   # 현재 차수의 회귀식 출력
    print(fit_eq)
    # 현재 차수의 회귀식을 이용하여 R² 값 계산 및 출력
    print(f'r²={r2_score(tm_ref, fit_eq(wl_list[0]))}')
    # print(f'run time : {run_time}s\n')   # 회귀에 걸린 시간 출력
    rsq_ref.append(r2_score(tm_ref, fit_eq(wl_list[0])))   # R² 값을 리스트에 저장
    # 현재 차수의 회귀식을 이용하여 그래프를 그리고, 라벨에 R² 값을 표시
    plt.plot(wl_ref, fit_eq(wl_ref), label=f'{p}th R² : {r2_score(tm_ref, fit_eq(wl_list[0]))}')

# 그래프 제목과 축 라벨 지정, 범례 표시
...
DC_bias = -2.0   # 초기 DC bias 값 설정
plt.subplot(2, 3, 3)   # 그래프를 그릴 위치 설정
rsq_ref = []
for j in range(6):   # 6개의 DC bias에 대해 반복
    # 참조 스펙트럼에서 회귀식을 이용하여 차분 값을 구하고, 그래프를 그림
    plt.plot(wl_ref, tm_ref - fit_eq(wl_ref))
    # DC bias 값을 이용하여 각각의 테스트 스펙트럼에서 차분 값을 구하고, 그래프를 그림
    plt.plot(wl_list[j], tm_list[j]-fit_eq(wl_list[j]), plot_color[j], label=f'DC_bias={DC_bias}')
    DC_bias += 0.5   # DC bias 값을 증가시킴
...
df = pd.DataFrame({
    ...
    'Rsq of Ref. spectrum (Nth)': rsq_ref[2],
    'Max transmission of Ref. spec. (dB)': np.max(arr_tm),
    'Rsq of IV': rsq_iv,
    'I at -1V [A]': cur[4],
    'I at 1V [A]': abs(cur[-1])
})
