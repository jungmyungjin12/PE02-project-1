...
plt.subplot(2,2,3) # 그래프를 1행 2열 디자인으로 구성 하고 이 plot 을 첫 번째 자리에 올림
...
for i in range(0,len(WLL)):   # DCBias key 값에 대응되는 요소들을 list 형태로 DC_list에 저장,
  if i == len(WLL) - 1:        # 참조값의 범례 제외해 응용과제 결과물과 일치키기 위하여 if 로 경우를 정해줌
    plt.subplot(2,2,1)            # 1행 2열의 디자인 형태중 2번째 자리에 plot함
    plt.plot(WLL[i], TRL[i])
  else:
    plt.subplot(2,2,1)            # 참조값의 범례가 아닌 경우에는 DCbias[V]로 범례를 표시해야 하기 때문에 라벨링을 함.
...
for i in range(1,11):
  fit3 = np.polyfit(WLL[-1], TRL[-1], i)  # polyfit을 이용하여 V와 절대값 I와의 그래프를 1000차로 근사하여 각 계수들을 fit2에 지정.
...
plt.text(1550, -12, 'R-squared ={}'.format(max(square_list)-1), fontdict={'size':6})
plt.text(1550, -13, 'R-squared ={}'.format(max(square_list)), fontdict={'color':  'red','size':6})


#print(square_list)
#print(max(square_list),square_list.index(max(square_list)))
